...
export function RedOverlar(center, length, className) {
...
RedOverlar.prototype = new BMap.Overlay();

// 实现初始化方法
RedOverlar.prototype.initialize = function (map) {
    const that = this;
    // 保存map对象实例
    this._map = map;
...
// 实现绘制方法
RedOverlar.prototype.draw = function () {
...
// 实现显示方法
RedOverlar.prototype.show = function () {
    if (this._div) {
        this._div.style.display = '';
    }
};
// 实现隐藏方法
RedOverlar.prototype.hide = function () {
    if (this._div) {
        this._div.style.display = 'none';
    }
};
// 添加自定义方法
RedOverlar.prototype.toggle = function () {
    if (this._div) {
